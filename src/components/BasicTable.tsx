...
export default function BasicTable({ people }) {
  return (
    <ul role="list" className="w-full divide-y divide-gray-100">
      {people.map((person) => (
        <li key={person.email} className="relative flex justify-between py-5 hover:bg-gray-50">
          <Link href={`/${person.id}`} className="relative flex flex-1 justify-between py-5 hover:bg-gray-50">
            ...
            <div className="flex items-center justify-between gap-x-4">
                ...
                <p className="text-sm leading-6 text-gray-900 whitespace-nowrap">Price: {person.displayPrice / 1e18 + ' ETH'}</p>
